...
public class NeuralNet implements Serializable {
	...
	private static final long serialVersionUID = -4552322183998690024L;
	...
	**/
	...
	public void setNeuronWeights(int layer, int index, double[][] weights) {
		Neuron neuron = (neurons.get(neuronsIndex[layer][index]));
		Edge edge;
		for (int i = 0; i < inputNum; i++) {
			edge = (neuron.getBackwardEdges().get(i));
			edge.setWeight(weights[i][0]);
		}
		for (int i = 0; i < outputNum; i++) {
			edge = (neuron.getForwardEdges().get(i));
